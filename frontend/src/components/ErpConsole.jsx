...
export default function ErpConsole({ user }) {
  ...
  // Form states
  const [prodForm, setProdForm] = useState({ name: '', current_stock: '', reorder_threshold: '', unit_cost: '', supplier_id: '' });
  const [custForm, setCustForm] = useState({ name: '', contact_channel: '', consent_status: 'opted_in', outstanding_due: '0' });
  const [suppForm, setSuppForm] = useState({ name: '', contact_channel: '' });
  const [saleForm, setSaleForm] = useState({ customer_id: '', product_id: '', quantity: '1' });
  ...
  const fetchData = async () => {
    if (!user) return;
    try {
      const pRes = await fetch(`${backendUrl}/products?business_id=${user.business_id}`);
      const pData = await pRes.json();
      setProducts(pData);

      const cRes = await fetch(`${backendUrl}/customers?business_id=${user.business_id}`);
      ...
      if (settingsRes.ok) {
        ...
        if (reminderForm.remind_who === 'Self' && !reminderForm.contact_channel) {
          setReminderForm(prev => ({ ...prev, contact_channel: emailVal || user.username || '' }));
        }
      }
    } catch (e) {
      console.error(e);
  ...
  useEffect(() => {
    if (reminderForm.remind_who === 'Self') {
      setReminderForm(prev => ({ ...prev, contact_channel: smtpSettings.smtp_email || user.username || '' }));
    } else {
  ...
  const handleAddProduct = async (e) => {
    e.preventDefault();
    if (!prodForm.name || !prodForm.current_stock || !prodForm.reorder_threshold || !prodForm.unit_cost) {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        setProdForm({ name: '', current_stock: '', reorder_threshold: '', unit_cost: '', supplier_id: '' });
        fetchData();
      }
    } catch (err) {
  ...
  const handleRecordSale = async (e) => {
    ...
    const items = [{ name: product.name, qty: parseInt(saleForm.quantity), price: product.unit_cost }];
  ...
  const handleCreateReminder = async (e) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        setReminderForm({ title: '', what_to_remind: '', remind_who: 'Self', contact_channel: '' });
      }
    } catch (err) {
    ...
    <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800 p-6 rounded-2xl shadow-xl space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between border-b border-slate-800 pb-4 gap-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold tracking-wide">ERP Data Management</h2>
        </div>
        <div className="flex flex-wrap items-center gap-2 bg-slate-950/80 p-1 rounded-xl border border-slate-800">
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'products' ? 'bg-indigo-600 text-slate-100' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'customers' ? 'bg-indigo-600 text-slate-100' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'suppliers' ? 'bg-indigo-600 text-slate-100' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'sale' ? 'bg-indigo-600 text-slate-100' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'smtp' ? 'bg-indigo-600 text-slate-100' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'reminder' ? 'bg-indigo-600 text-slate-100' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
      ...
      {activeTab === 'products' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-3">
            <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Current Inventory Ledger</h3>
                ...
                <tbody className="divide-y divide-slate-800">
                  {Array.isArray(products) && products.map(p => (
                    <tr key={p.id} className="hover:bg-slate-900/20">
                      ...
                      <td className="p-3 text-slate-300">{p.current_stock} Units</td>
                      <td className="p-3 text-slate-400">{p.reorder_threshold} Units</td>
                      <td className="p-3 text-slate-300">₹{p.unit_cost.toFixed(2)}</td>
                      <td className="p-3">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          ...
                          {p.current_stock < p.reorder_threshold ? 'Low Stock' : 'Healthy'}
                        </span>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="bg-slate-950/40 p-5 rounded-xl border border-slate-800/80 space-y-4">
            <h3 className="text-sm font-semibold text-slate-200">Add New Product</h3>
            <form onSubmit={handleAddProduct} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Initial Stock</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Reorder Limit</label>
                  <input 
                    type="number" 
                    value={prodForm.reorder_threshold} 
                    onChange={e => setProdForm({...prodForm, reorder_threshold: e.target.value})}
                    placeholder="e.g. 15" 
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-slate-200 focus:outline-none focus:border-indigo-500"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Unit Price (₹)</label>
                  <input 
                    type="number" 
                    value={prodForm.unit_cost} 
                    onChange={e => setProdForm({...prodForm, unit_cost: e.target.value})}
                    placeholder="e.g. 120" 
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-slate-200 focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div>
                  <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Supplier</label>
                  <select 
      ...
      {activeTab === 'customers' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-3">
            <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Customer Ledger Accounts</h3>
            <div className="overflow-x-auto rounded-xl border border-slate-800">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-950/60 text-slate-400 border-b border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800">
                  {Array.isArray(customers) && customers.map(c => (
                    <tr key={c.id} className="hover:bg-slate-900/20">
                      ...
                      <td className="p-3 text-slate-400">{c.contact_channel}</td>
                      <td className="p-3 font-medium text-amber-400">₹{c.outstanding_due.toFixed(2)}</td>
                      <td className="p-3 text-slate-300">{c.last_order_date || 'No Orders'}</td>
                      <td className="p-3">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                          ...
                          {c.consent_status === 'opted_in' ? 'Notification Opt-in' : 'Opt-out'}
                        </span>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="bg-slate-950/40 p-5 rounded-xl border border-slate-800/80 space-y-4">
            <h3 className="text-sm font-semibold text-slate-200">Add New Customer</h3>
            <form onSubmit={handleAddCustomer} className="space-y-3">
              ...
              <div>
                <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Contact Email (for Receipts)</label>
                <input 
                  type="email" 
                  required
                  value={custForm.contact_channel} 
                  onChange={e => setCustForm({...custForm, contact_channel: e.target.value})}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Consent</label>
                  <select 
                    value={custForm.consent_status} 
                    onChange={e => setCustForm({...custForm, consent_status: e.target.value})}
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-slate-400 focus:outline-none focus:border-indigo-500"
                  >
                ...
                <div>
                  <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Initial Due (₹)</label>
                  <input 
                    type="number" 
                    value={custForm.outstanding_due} 
                    onChange={e => setCustForm({...custForm, outstanding_due: e.target.value})}
                    placeholder="0" 
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-slate-200 focus:outline-none focus:border-indigo-500"
                  />
                </div>
      ...
      {activeTab === 'suppliers' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-3">
            <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Registered Suppliers</h3>
            <div className="overflow-x-auto rounded-xl border border-slate-800">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-950/60 text-slate-400 border-b border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800">
                  {Array.isArray(suppliers) && suppliers.map(s => (
                    <tr key={s.id} className="hover:bg-slate-900/20">
                      ...
                      <td className="p-3 text-slate-400">{s.contact_channel}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-slate-950/40 p-5 rounded-xl border border-slate-800/80 space-y-4">
            <h3 className="text-sm font-semibold text-slate-200">Add New Supplier</h3>
            <form onSubmit={handleAddSupplier} className="space-y-3">
              <div>
                <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Supplier Name</label>
                <input 
                  type="text" 
                  required
                  value={suppForm.name} 
              ...
              <div>
                <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Supplier Email Address</label>
                <input 
                  type="email" 
                  required
                  value={suppForm.contact_channel} 
                  onChange={e => setSuppForm({...suppForm, contact_channel: e.target.value})}
      ...
      {activeTab === 'sale' && (
        <div className="max-w-xl mx-auto bg-slate-950/40 p-6 rounded-xl border border-slate-800/80 space-y-4">
          ...
          <form onSubmit={handleRecordSale} className="space-y-4">
            <div>
              <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Customer (Optional)</label>
              <select 
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="col-span-2">
                <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Select Product</label>
                <select 
                ...
                >
                  ...
                  {Array.isArray(products) && products.map(p => (
                    <option key={p.id} value={p.id}>{p.name} (Stock: {p.current_stock} units, ₹{p.unit_cost}/unit)</option>
                  ))}
                </select>
              </div>
              
              <div>
                <label className="block text-[10px] text-slate-400 uppercase font-semibold mb-1">Quantity</label>
                <input 
                  type="number" 
            ...
            {saleForm.product_id && (
              <div className="bg-slate-950/80 p-3 rounded-lg border border-slate-900 text-xs flex justify-between items-center text-slate-400 font-mono">
                ...
                <span className="text-emerald-400 font-bold text-sm">
                  ₹{(parseFloat(products.find(p => p.id === saleForm.product_id)?.unit_cost || 0) * parseInt(saleForm.quantity || 0)).toFixed(2)}
                </span>
              </div>
            )}
      ...
      {activeTab === 'smtp' && (
        <div className="max-w-xl mx-auto bg-slate-950/40 p-6 rounded-xl border border-slate-800/80 space-y-4">
          <div className="flex items-center gap-2 text-indigo-400">
            ...
            <h3 className="text-sm font-semibold">SMTP Outgoing Email Settings</h3>
          </div>
          <p className="text-[11px] text-slate-400 leading-relaxed">
            Configure SMTP to send actual email notifications to suppliers and customers. If left blank, notifications will fall back to local logging inside <code>backend/sent_emails.log</code> without interrupting the application flow.
          </p>
      ...
      {activeTab === 'reminder' && (
        <div className="max-w-xl mx-auto bg-slate-950/40 p-6 rounded-xl border border-slate-800/80 space-y-4">
          <div className="flex items-center gap-2 text-indigo-400">
            ...
            <h3 className="text-sm font-semibold">Set Operational Custom Reminder</h3>
          </div>
          <p className="text-[11px] text-slate-400 leading-relaxed">
            Create custom reminders that get processed by the AURA Copilot system. Clicking "✓ Execute" on the reminder drafts will push the alert email to the recipient address.
          </p>
          <form onSubmit={handleCreateReminder} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select 
                  ...
                  onChange={e => setReminderForm({ ...reminderForm, remind_who: e.target.value })}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-slate-400 focus:outline-none focus:border-indigo-500"
                >
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={e => setReminderForm({ ...reminderForm, contact_channel: e.target.value })}
            ...
            {reminderForm.remind_who === 'Customer' && Array.isArray(customers) && customers.length > 0 && (
              <div className="bg-slate-950/80 p-2.5 rounded-lg border border-slate-900 text-[10px] text-slate-400 space-y-1">
                <span className="font-bold uppercase tracking-wider block text-slate-500 mb-0.5">Quick Fill Customer Contact:</span>
                <div className="flex flex-wrap gap-1.5">
                  {customers.map(c => (
                    <button
                      key={c.id}
                      type="button"
                      onClick={() => setReminderForm(prev => ({ ...prev, contact_channel: c.contact_channel }))}
            ...
            {reminderForm.remind_who === 'Supplier' && Array.isArray(suppliers) && suppliers.length > 0 && (
              <div className="bg-slate-950/80 p-2.5 rounded-lg border border-slate-900 text-[10px] text-slate-400 space-y-1">
                <span className="font-bold uppercase tracking-wider block text-slate-500 mb-0.5">Quick Fill Supplier Contact:</span>
                <div className="flex flex-wrap gap-1.5">
                  {suppliers.map(s => (
                    <button
                      key={s.id}
                      type="button"
                      onClick={() => setReminderForm(prev => ({ ...prev, contact_channel: s.contact_channel }))}
                      className="px-2 py-0.5 bg-slate-900 hover:bg-slate-850 text-indigo-400 border border-slate-800 rounded text-[10px] transition-all"
                    >
                      {s.name}
                    </button>
            ...
            <div>
              ...
              <textarea 
                ...
                onChange={e => setReminderForm({ ...reminderForm, what_to_remind: e.target.value })}
