...
export default function App() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      // Fetch stats
      const statsRes = await fetch(`${backendUrl}/dashboard/stats?business_id=${user.business_id}`);
      const statsData = await statsRes.json();
      setStats(statsData);

      // Fetch charts
      const chartsRes = await fetch(`${backendUrl}/dashboard/charts?business_id=${user.business_id}`);
      ...
      if (actions.length > 0 && actionsData.length > actions.length) {
        ...
        if (newAction.status === 'proposed') {
          toast.success(`⚡ ${newAction.agent_type.toUpperCase()} Agent: Action proposed!`, {
            duration: 4000,
            icon: '🤖',
            style: {
              background: '#0f172a',
  ...
  const handleExecuteConfirm = async (id, payload) => {
    try {
      ...
      if (data.status === 'success') {
        if (data.email_sent) {
          toast.success(`Action Executed & Email Dispatched to ${data.recipient}!`, {
            style: { background: '#047857', color: '#ecfdf5' },
            duration: 5000
          });
        } else {
          toast.success("Action Executed (Notified locally in sent_emails.log)!", {
  ...
  const handleIgnore = async (id) => {
    try {
      const res = await fetch(`${backendUrl}/actions/${id}/ignore`, { method: 'POST' });
      const data = await res.json();
      if (data.status === 'success') {
  ...
  const handleGenerateSummary = async () => {
    ...
    toast.loading(`Analyzing operational brief (Last ${dateRange} days)...`, { id: 'summary-toast' });
    try {
  ...
  const triggerHelper = async (endpoint, label) => {
    ...
    try {
      const res = await fetch(`${backendUrl}/demo/${endpoint}?business_id=${user.business_id}`, { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
        toast.success(`Agent Active! Checking database...`, { id: 'trigger-toast', duration: 2000 });
        setTimeout(fetchDashboardData, 1000);
      } else {
        toast.error(data.detail || "Trigger simulation failed. Add records to ERP first.", { id: 'trigger-toast' });
      }
    } catch (e) {
      toast.error("Trigger simulation failed. Add records to ERP first.", { id: 'trigger-toast' });
    }
  };
    ...
    try {
      const res = await fetch(`${backendUrl}/webhooks/voice`, {
        ...
        body: JSON.stringify({
          ...
          transcript: "Hey, please send 3 packets of Aashirvaad Atta and 1 kg of Moong Dal to my house.",
          business_id: user.business_id
        })
      });
      const data = await res.json();
      if (res.ok) {
        toast.success(`Booking Confirmed! Added to Orders & Feed.`, { id: 'voice-toast', duration: 4000 });
        setTimeout(fetchDashboardData, 500);
      } else {
        toast.error(data.detail || "Voice simulation failed. Set up inventory first.", { id: 'voice-toast' });
      }
    } catch (e) {
      toast.error("Voice simulation failed. Set up inventory first.", { id: 'voice-toast' });
    }
  };
      ...
      <header className="bg-slate-950/80 backdrop-blur-md border-b border-slate-800 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-wider text-slate-100">{user.name}</h1>
              <p className="text-[10px] text-indigo-400 font-semibold tracking-widest uppercase">AURA Business Nervous System</p>
            </div>
          </div>
          ...
          <div className="flex items-center bg-slate-950/80 border border-slate-800 p-1 rounded-xl">
            <button 
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
                view === 'dashboard' ? 'bg-indigo-600 text-slate-100 shadow-lg shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button 
              onClick={() => setView('erp')}
              className={`px-4 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
                view === 'erp' ? 'bg-indigo-600 text-slate-100 shadow-lg shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
          ...
          {stats && stats.agents && (
            <div className="hidden lg:flex items-center gap-6 text-xs border-l border-slate-800 pl-6">
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${stats.agents.restock === 'Active' ? 'bg-emerald-500 animate-ping' : 'bg-slate-500'}`}></span>
                <span className="text-slate-400 font-medium">Restock:</span>
                <span className={stats.agents.restock === 'Active' ? 'text-emerald-400 font-bold' : 'text-slate-500'}>
                  {stats.agents.restock}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${stats.agents.collections === 'Active' ? 'bg-emerald-500 animate-ping' : 'bg-slate-500'}`}></span>
                <span className="text-slate-400 font-medium">Collections:</span>
                <span className={stats.agents.collections === 'Active' ? 'text-emerald-400 font-bold' : 'text-slate-500'}>
                  {stats.agents.collections}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${stats.agents.winback === 'Active' ? 'bg-emerald-500 animate-ping' : 'bg-slate-500'}`}></span>
                <span className="text-slate-400 font-medium">Winback:</span>
                <span className={stats.agents.winback === 'Active' ? 'text-emerald-400 font-bold' : 'text-slate-500'}>
                  {stats.agents.winback}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${stats.agents.voice === 'Active' ? 'bg-emerald-500 animate-ping' : 'bg-slate-500'}`}></span>
                <span className="text-slate-400 font-medium">Voice:</span>
                <span className={stats.agents.voice === 'Active' ? 'text-emerald-400 font-bold' : 'text-slate-500'}>
                  {stats.agents.voice}
                </span>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-8 space-y-8">
                ...
                {summary ? (
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-slate-500 text-[10px] uppercase font-bold tracking-wider mr-2">Quick Commands:</span>
                      {summary.recommendations.map((rec, idx) => (
                        <button 
                          key={idx}
                          onClick={() => {
                            if (rec.toLowerCase().includes('stock')) triggerHelper('trigger-stock-drop', 'Restock');
                            else if (rec.toLowerCase().includes('reminder')) triggerHelper('trigger-customer-due', 'Collections');
                  ...
                  <p className="text-slate-400 text-xs font-medium max-w-xl">
                    Ready to compile a unified operational brief. Select your range parameter and run scans.
                  </p>
                )}
              </div>
              ...
              <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800 p-6 rounded-2xl shadow-xl animate-slide-in">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold tracking-wide">Monthly Financial Performance Ledger</h3>
                  </div>
                ...
                <div className="mt-4 flex items-center justify-between bg-slate-950/40 p-3.5 rounded-xl border border-slate-800/50 text-xs">
                  <span className="text-slate-400 font-medium">Weekly Comparison:</span>
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 rounded-full flex items-center gap-1 text-[10px] font-bold ${
                      financials.weekly_comparison.is_positive ? 'bg-emerald-500/15 text-emerald-400' : 'bg-rose-500/15 text-rose-400'
                    }`}>
        ...
        <div className="bg-slate-950/60 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-4">
          ...
          <p className="text-xs text-slate-400 max-w-2xl leading-relaxed">
            Click these triggers to immediately mutate database states. Within 10 seconds, the orchestrator will notice the change, show top-right toasts, and slide actions into the feed.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pt-2">
            ...
            <button 
              onClick={() => triggerHelper('trigger-customer-due', 'Collections')}
